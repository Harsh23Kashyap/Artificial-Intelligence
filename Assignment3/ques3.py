#Apply A*searching algorithmby taking H(n): number of correctly placed tiles 
# in the current statenas compared to the goal node.as the heuristic function..
...
def BFS(state,goal):
    ...
    counter=0   #count no. of iterations and check distance from initial state
    while(len(queue)>0 and counter<10000):
        ...
        if (state in queue):
            break
        actualvisit = actualvisit + [state]
        k,l=findzero(state,0)
        print("\n\nLoop ", counter)
        print("STATE---",state)
        #UP
        ns=up(k,l,state)
        ns[2]=state[2]+1  #updating the distance
        ...
        if(equalitycheck(goal,ns)):
            ...
            if (not(ns in visited)):
                ns[0]=state[2]+heuristic(goal,ns)
                visited=visited+[ns]
                queue= queue+[ns]
    ...
    print("\nTotal time = ",counter)
    if(check):
        actualvisit=actualvisit+[ns]
        print("\nActually Visited matrices are - : ",actualvisit)
    if(not(check)):
        print("\nNot Found ")   
...
if (__name__== "__main__"):
    ...
    state=[0,[[2,0,3],[1,8,4],[7,6,5]],0]
    

    goal=[0,[[1,2,3],[8,0,4],[7,6,5]],9]
    #goal=[[1,3,0],[8,2,4],[7,6,5]]
    #goal=[[2,8,1],[7,6,5],[0,4,3]]
    #goal=[4,[[2,1,3],[4,0,5],[6,7,8]]]
    res=state[2]+heuristic(state,goal)
    goal[0]=res
